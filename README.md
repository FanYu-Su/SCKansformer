...
- 18.特别的，我在代码里补全了原来就配置过的其他已开源模型的代码，包括VGG和VIT和ResNet等模型，方便大家进行对比实验，其中WCBsNet由于官方代码缺失，我只针对论文中具有的框架，简单搭建了一个卷积架构，但是和官方的WCBsNet有很明显的不同，仅供参考。
- 19.目前更新了一下kansformer_kan的训练模型，更新了一下kan的参数调整；目前调用kansformer_kan将会采用8网格5阶的kan网络进行训练；但这将会极大程度的影响训练时间，所以请注意使用；
- 20.应该是最后一次更新，这次是针对MSTAR数据集进行移植适配补充，调整download.ipynb文件名字，修改为tools.ipynb，由于数据集是其他论文中下载，不确定是否适配官方下载的数据集，为避免版权问题，故而不提供数据集下载链接，请自行下载MSTAR数据集进行使用，具体的使用方法可以参考notebook中的tools.ipynb文件
